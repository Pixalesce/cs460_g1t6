...
BUCKET_NAME = 'sscompanion-images'  # Replace with your bucket name
...
def upload_file_to_s3(file_path, bucket_name, s3_file_name):
    try:
        s3.upload_file(file_path, bucket_name, s3_file_name, ExtraArgs={'ContentType': 'image/jpeg'})
...
motion_sensitivity = 500  # Adjust based on the level of motion significance
...
while True:
    ...
    if motion_detected:
        if motion_start_time is None:
            ...
            # Run the upload
            upload_file_to_s3(image_path, BUCKET_NAME, image_path)
            motion_start_time = None  # Reset the timer after taking a picture
    else:
        motion_start_time = None  # Reset if motion stops b4 duration threshold
