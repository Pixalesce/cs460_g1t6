...
# Configure logging
logging.basicConfig(
    filename=os.path.join(os.path.dirname(__file__), "logs/pairing.log"),
    level=logging.INFO,
...
def save_mac_address(mac_address):
    ...
    tmp = mac_address.split("/")[-1].split("_")
    mac_address = ":".join(tmp[1:])
    print(mac_address)
    devices = [mac_address]
        ...
        with open("device_mac.txt", "r") as input_file:
            for line in input_file:
                ...
                if line not in devices:
                    devices.append(line)
    with open("device_mac.txt", "w") as output_file:
        for address in devices:
            output_file.write(address + "\n")
    logging.info(f"Saved MAC address: {mac_address}")
    print(f"Saved MAC address: {mac_address}")
...
class Agent(dbus.service.Object):
    ...
    def DisplayPasskey(self, device, passkey, entered):
        ...
        logging.info("DisplayPasskey (%s, %06u entered %u)" % (device, passkey, entered))
